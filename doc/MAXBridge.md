...
- Доступен в мини-приложении через `initDataUnsafe.start_param`
- Если payload превышает 512 символов или содержит недопустимые символы — удаляется из ответа

**Диплинк для шеринга:**
```
https://max.ru/:share?text=<URL-encoded>
```
Открывает экран «Отправить в MAX» — пользователь выбирает чат и отправляет сообщение.

### Шеринг медиа через бота

1. Бот отправляет контент пользователю через `POST /messages`
2. Мини-приложение получает `mid` сообщения
3. Вызов: `window.WebApp.shareMaxContent({ mid, chatType: 'DIALOG' | 'CHAT' })`

### Настройка мини-приложения на платформе

1. Платформа MAX для партнёров → Чат-боты → Настроить
2. Вставить URL мини-приложения (только `https://`, до 1024 символов)
3. Выбрать вид кнопки: Открыть / Старт / Играть
4. Хостинг: GitHub Pages, Vercel и т.д.
