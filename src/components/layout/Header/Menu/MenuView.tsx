...
export const MenuView: React.FC<MenuViewProps> = ({
    ...
    return (
        <div className={`fixed inset-0 z-[1000] ${isOpen ? 'pointer-events-auto' : 'pointer-events-none'}`}>
            <div
                className={`absolute inset-0 bg-[#050510]/80 backdrop-blur-md transition-opacity duration-500 ${isOpen ? 'opacity-100' : 'opacity-0'}`}
                onClick={onClose}
            />

            <div className={`absolute top-0 right-0 h-full bg-[#08081a] border-l border-white/10 shadow-2xl transition-all duration-500 ease-in-out ${
                isOpen ? 'translate-x-0' : 'translate-x-full'
            } ${showKnowledge ? 'w-full' : 'w-[85%] max-w-[360px]'}`}>

                <div className="flex flex-col h-full relative overflow-hidden">

                    {/* Кнопка назад для Базы Знаний */}
                    <div className={`absolute top-6 left-6 z-[130] transition-all duration-500 ${showKnowledge ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4 pointer-events-none'}`}>
                        <button
                            ...
                            className="flex items-center gap-2 text-fuchsia-500 bg-fuchsia-500/10 px-4 py-2 rounded-full backdrop-blur-xl border border-fuchsia-500/20 shadow-lg"
                        >
                            <ChevronLeft size={18} />
                            <span className="text-[10px] font-black uppercase tracking-[0.2em]">Вернуться в меню</span>
                        </button>
                    </div>

                    {/* Слой Базы Знаний (Независимый скролл) */}
                    <div className={`absolute inset-0 z-[120] overflow-y-auto no-scrollbar transition-all duration-500 bg-[#08081a] ${showKnowledge ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-full pointer-events-none'}`}>
                    ...
                    <div
                        ...
                        className={`flex-1 overflow-y-auto no-scrollbar scroll-smooth transition-all duration-500 ${!showKnowledge ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
                    >
                        <div className="p-6 pt-12">
                            ...
                            <div className="space-y-8">
                                ...
                                <div className="space-y-3" ref={creatorRef}>
                                    ...
                                    <div className={`border border-white/10 rounded-2xl overflow-hidden transition-all duration-500 ${isLegalOpen ? 'bg-white/5 border-fuchsia-500/30' : 'bg-white/[0.02]'}`}>
                                        <button onClick={toggleLegal} className="w-full p-5 flex items-center justify-between">
                                            <div className="flex items-center gap-3 text-fuchsia-400"><Building2 size={18} /><span className="text-[10px] font-black uppercase tracking-widest">ИП АНТОНОВ А.О.</span></div>
                                            <ChevronDown size={16} className={`text-white/20 transition-transform duration-500 ${isLegalOpen ? 'rotate-180' : ''}`} />
                                        </button>
                                            ...
                                            <div className="px-5 pb-5 space-y-4">
                                                <div className="flex items-center gap-4 pt-4 border-t border-white/5">
                                                    <div onClick={() => setIsPhotoModalOpen(true)} className="w-12 h-12 rounded-xl bg-black border border-fuchsia-500/30 overflow-hidden relative group cursor-pointer">
                                                        <img src={photoPath} alt="Founder" className="w-full h-full object-cover opacity-60 group-hover:opacity-100 transition-opacity" />
                                                        <div className="absolute inset-0 bg-fuchsia-500/20 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity"><Maximize2 size={12} className="text-fuchsia-400" /></div>
                                                    </div>
                                                    <div>
                                                ...
                                                <div className="space-y-2 bg-black/40 p-3 rounded-xl border border-white/5 text-[9px] font-mono">
                                                    <div className="flex justify-between items-center"><span className="text-white/30 uppercase">ИНН</span><div className="flex items-center gap-2 text-white/70">{inn}<button onClick={() => handleCopy(inn, 'inn')}>{copiedField === 'inn' ? <Check size={10} className="text-fuchsia-400"/> : <Copy size={10}/>}</button></div></div>
                                                    <div className="flex justify-between items-center"><span className="text-white/30 uppercase">ОГРНИП</span><div className="flex items-center gap-2 text-white/70">{ogrnip}<button onClick={() => handleCopy(ogrnip, 'ogrnip')}>{copiedField === 'ogrnip' ? <Check size={10} className="text-fuchsia-400"/> : <Copy size={10}/>}</button></div></div>
                                                    <div className="flex items-start justify-between pt-1">
                                                        <span className="text-white/30 uppercase">Адрес</span>
                                                        <div className="flex items-center gap-1 text-white/70 text-right max-w-[140px]"><span>{address}</span></div>
                                                    </div>
                                                    <div className="flex items-center justify-between pt-1">
                                                        <span className="text-white/30 uppercase">E-mail</span>
                                                        <a href={`mailto:${supportEmail}`} className="flex items-center gap-1 text-fuchsia-400/80 hover:text-fuchsia-400 transition-colors"><span>{supportEmail}</span></a>
                                                    </div>
                                                    <div className="pt-2 border-t border-white/5 flex justify-between items-center">
                                                        <span className="text-white/30 uppercase">Donate</span>
                                                        <a href={donatePageUrl} target="_blank" rel="noreferrer" className="text-fuchsia-400 flex items-center gap-1 font-bold"><Heart size={10} fill="currentColor" className="animate-pulse"/> ЮMoney</a>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="space-y-2 pb-10">
                                        <button onClick={() => onDocSelect('terms')} className="w-full p-4 rounded-xl bg-white/[0.02] border border-white/5 flex items-center justify-between group active:bg-white/5 transition-all">
                                        ...
                                        <button onClick={() => onDocSelect('privacy')} className="w-full p-4 rounded-xl bg-white/[0.02] border border-white/5 flex items-center justify-between group active:bg-white/5 transition-all">
                                            <div className="flex items-center gap-3">
            ...
            {isPhotoModalOpen && (
                <div className="fixed inset-0 z-[2000] bg-black/95 backdrop-blur-2xl flex flex-col items-center justify-center p-6 animate-in fade-in duration-300" onClick={() => setIsPhotoModalOpen(false)}>
